(function() {
  ...
  Area = (function() {
    ...
    Area.prototype.in_area = function(ball) {
      if (ball.x === this.x && ball.y === this.y) {
        ball.velocity.x = 0;
        ball.velocity.y = 0;
        return ball.opacity = 0.99;
      } else {
        ball.velocity.x = ball.velocity.x * config.area_decay + (this.x - ball.x);
        return ball.velocity.y = ball.velocity.y * config.area_decay + (this.y - ball.y);
      }
    };
  ...
  Ball = (function(_super) {
    ...
    Ball.prototype.touch = function(event, type) {
      ...
      if (type === 'start') {
        ...
        frames = this.game.frame - this.touch_start.frame;
        this.velocity.x = (event.x - this.touch_start.x) / frames;
        return this.velocity.y = (event.y - this.touch_start.y) / frames;
      }
    };
    ...
    Ball.prototype.update = function() {
      ...
      if (this.opacity < 1.0) {
        ...
        if (this.opacity < 0.01) this.full_reset();
        return;
      }
      ...
      if (this.release) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          area = _ref[_i];
          if (this.within(area, 32)) area.in_area(this);
        }
  ...
  MyGame = (function(_super) {
    ...
    function MyGame(width, height) {
      ...
      this.onload = function() {
        ...
        this.areas = [new Area(this, 0, this.width / 2 - 32, 32, 64, 64), new Area(this, 1, 32, this.height - 96, 64, 64), new Area(this, 2, this.width - 96, this.height - 96, 64, 64)];
