(function() {
  ...
  window.StageView = (function(_super) {
    ...
    StageView.prototype.initialize = function() {
      ...
      return this.s_template = _.template($('#scene-template-' + ("" + (this.model.get('format')))).html());
    };
    ...
    StageView.prototype.render = function() {
      ...
      if (this.model.scenes.length > 0) {
        $("#scenes").empty();
        $("#scenes").append($(_tableheads[this.model.get('format')]));
    ...
    StageView.prototype.setTitle = function() {
      ...
      if ((this.model.get('format')) === 'multichoice') {
        el_title.append($("<span/>").addClass("label success").text("New"));
        return el_title.append(($("<span/>").addClass("label important").text("Play")).click(function() {
          return location.href = "/dumon/\#" + _this.model.id;
        }));
      }
    };
